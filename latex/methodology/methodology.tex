...
to authenticate users strongly.
...
\texttt{Unauthorized} status code if the request is invalid.
\footnote{
...
metadata sent with the request, and then the user gets stored in the database,
...
} but also to gather the necessary bits of the key reconstruction
(See Section~\ref{subsec:password-encryption}).

\subsection{Cors}\label{subsec:cors}
Cross-Origin Resource Sharing (CORS) must be configured correctly with the
server and client running separately on different ports.
...
some information about the origin, accepted Content-Type, and similar to the
actual request.
...
\begin{itemize}
  ...
  \item \textbf{Secure}:
  We applied the HttpOnly attribute to ensure that the cookie was only
  accessible through HTTP or HTTPS protocols, and we transmitted it only over a
...
one described above.
Furthermore, since the passwords should be a randomly generated string unknown
to the applicant, we cannot hash it and compare the input from the user.
...
encoding.
An example of this can be seen in~\ref{subsec:base64-and-ciphertext}.
